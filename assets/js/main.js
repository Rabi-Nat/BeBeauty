(function ($) {
  ...
  */
  ...
  // بارگذاری sidebar
fetch('./assets/includes/sidemenu.html')
.then(response => {
    if (!response.ok) throw new Error('خطا در بارگذاری sidebar');
    return response.text();
})
.then(data => {
    document.querySelector('div.sidemenu').innerHTML = data;
})
...
$(document).ready(function() {
    // Configure Slick Slider for RTL
    $('.slick-slider').slick({
        rtl: true,
        // ... existing code ...
    });

    // Configure DatePicker for RTL
    ...
    // Configure Magnific Popup for RTL
    $('.popup-image').magnificPopup({
        type: 'image',
    ...
    $('.vs-menu-wrapper .vs-mobile-menu ul li.menu-item-has-children > a').append('<i class="fas fa-chevron-left"></i>');
    ...
    // Apply RTL Price Format
    $('.price').each(function() {
        var price = $(this).text();
        $(this).text(formatPrice(price));
    });

  // RTL Calendar initialization removed: persianDatepicker not used for booking inputs per request.
});

// اشتراک‌گذاری رسید مالی
$(document).ready(function() {
  $(document).on('click', '.send-receipt-btn', function(e) {
    e.stopPropagation();
    // بستن منوهای دیگر
    $('.share-menu').hide();
    // نمایش منوی مربوط به این دکمه
    var $btn = $(this);
    var $menu = $btn.next('.share-menu');
    $menu.show();
    // گرفتن اطلاعات ردیف جدول
    var $tr = $btn.closest('tr');
    var invoice = $tr.children().eq(1).text().trim();
    var customer = $tr.children().eq(2).text().trim();
    var amount = $tr.children().eq(5).text().trim();
    var text = `رسید پرداخت برای ${customer}، شماره فاکتور ${invoice}، مبلغ ${amount}`;
    $menu.find('.share-whatsapp').attr('href', `https://wa.me/?text=${encodeURIComponent(text)}`);
    $menu.find('.share-telegram').attr('href', `https://t.me/share/url?text=${encodeURIComponent(text)}`);
  });
  // بستن منو با کلیک بیرون
  $(document).on('click', function(e) {
    if (!$(e.target).closest('.share-menu').length) {
      $('.share-menu').hide();
    }
  });
});

// Profile image upload and preview for profile71.html
